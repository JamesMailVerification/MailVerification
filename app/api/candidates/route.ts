...
export async function POST(request: Request) {
  ...
  for (const item of body.candidates ?? []) {
    const values = {
      ...
      location: String(item.location ?? "").slice(0, 100), date: String(item.date ?? ""), endDate: String(item.endDate ?? item.date ?? ""),
